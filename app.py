    ...
    for _, row in filtered.iterrows():
        ...
        with st.container():
            ...
            # ── Vista normal ──────────────────────────────────────────────────
                ...
                desc_part  = (f"<br><small style='opacity:.6;'>{desc}</small>") if desc else ""
                html = (
                    f"<div class='tx-card' style='border-left:4px solid {color};"
                    f"margin:.2rem 0;'>"
                    f"<div><strong>{category}</strong>"
                    f"<span style='opacity:.5;font-size:.8rem;margin-left:.6rem;'>"
                    f"{fecha} · {row['type']}</span>"
                    f"{desc_part}</div>"
                    f"<div style='text-align:right;white-space:nowrap;'>"
                    f"<strong style='color:{color};'>{amount_str}</strong>"
                    f"</div></div>"
                )
