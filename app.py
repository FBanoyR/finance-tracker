...
# ── Session helpers (query params) ───────────────────────────────────────────
SESSION_DAYS = 30
SESSION_PARAM = "s"   # URL param: ?s=TOKEN

def _session_login(storage) -> bool:
    """
    Lee el token de st.query_params, lo valida en sessions.json y loguea.
    Esto funciona 100% en Streamlit Cloud porque query_params se leen
    ...
    sessions, sha = storage.get_sessions()
    ...
    if datetime.utcnow().isoformat() > exp:
        sessions.pop(token, None)
        storage.save_sessions(sessions, sha)
    ...
    st.session_state.session_token = token   # guardar para logout
    return True

def _create_session(storage, username: str) -> str:
    """Genera token, lo guarda en sessions.json y retorna la URL con el token."""
    token   = _secrets_mod.token_urlsafe(32)
    expires = datetime.utcnow() + timedelta(days=SESSION_DAYS)

    sessions, sha = storage.get_sessions()
    # Limpiar sesiones viejas del mismo usuario
    sessions = {
        k: v for k, v in sessions.items()
        if not (v.get("username") == username and
                v.get("expires", "") < datetime.utcnow().isoformat())
    }
    sessions[token] = {
        "username": username,
...
def _clear_session(storage):
    """Elimina el token activo de sessions.json y limpia la URL."""
...
def _render_sidebar():
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Cerrar sesión", use_container_width=True, key="btn_logout"):
            _clear_session(st.session_state.storage)
...
def page_login():
    ...
    with col:
        ...
        remember_me = st.checkbox("🔒 Recordarme en este dispositivo (30 días)", key="li_remember")
        st.markdown("")
        ...
        if st.button("Entrar →", use_container_width=True, type="primary", key="btn_login"):
            if not username or not password:
                ...
                if username in users and users[username]["password"] == _hash(password):
                    ...
                    st.session_state.page      = "home"
                    if remember_me:
                        token = _create_session(storage, username)
    ...
    for _, row in df_user.sort_values("date", ascending=False).head(8).iterrows():
        ...
        st.markdown(
            f"""<div class="tx-card" style="border-left:4px solid {color};">
                ...
                {"<br><span style='color:var(--text-color);opacity:.6;font-size:.82rem;'>" + desc + "</span>" if desc else ""}
                </div>
                <div style="text-align:right;">
                    <strong style="color:{color};font-size:1.05rem;">${row['amount']:,.0f}</strong>
                    <br><span style="color:var(--text-color);opacity:.5;font-size:.8rem;">{fecha} · {row['type']}</span>
                </div></div>""",
            unsafe_allow_html=True)
...
def _get_app_url() -> str:
    """
    Retorna la URL base de la app.
    Prioridad: secret APP_URL → URL del navegador via JS → fallback genérico.
    """
    try:
        return st.secrets["APP_URL"].rstrip("/")
    except Exception:
        pass
    # Intentar leer la URL actual del navegador
    try:
    ...
    with tab_new:
        ...
        if st.button("✅ Crear grupo", type="primary", use_container_width=True, key="btn_create_group"):
            if not g_name.strip():
                ...
                invite_link = f"{base_url}/?group={gid}&join=1" if base_url else f"?group={gid}&join=1"
                st.success(f"✅ Grupo **{g_name}** creado!")
                st.markdown("**🔗 Enlace de invitación:**")
                st.code(invite_link, language=None)
                st.info("📋 Copia este enlace y compártelo. Los miembros deben tener cuenta en la app para unirse.", icon="ℹ️")
                st.caption("⚠️ Si el enlace no funciona, verifica que APP_URL en los Secrets de Streamlit Cloud sea la URL exacta de tu app.")
...
def page_group_detail():
    ...
    with tab_exp:
        ...
        if st.session_state.get("group_adding_expense"):
            with st.form("form_group_expense"):
                st.markdown("#### ➕ Nuevo gasto del grupo")
                fe1, fe2 = st.columns(2)
                with fe1:
                    e_desc   = st.text_input("📝 Descripción", placeholder="Hotel, cena, gasolina…")
                    e_amount = st.number_input("💵 Monto total", min_value=0.0, step=1000.0, format="%.2f")
                    e_date   = st.date_input("📅 Fecha", value=date.today())
                with fe2:
                    e_paid_by = st.selectbox("👤 ¿Quién pagó?", members)
                    e_cat     = st.selectbox("🏷️ Categoría", [
                        "🍽️ Comida", "🏨 Alojamiento", "🚗 Transporte",
                        "🎬 Entretenimiento", "🛍️ Compras", "💊 Salud",
                        "🎁 Regalos", "📋 Otro",
                    ])
                    e_split   = st.selectbox("⚖️ Dividir", ["Partes iguales", "Por porcentaje", "Montos específicos"])

                st.markdown("**¿Quiénes participaron?**")
                e_parts_sel = st.multiselect("Participantes", members, default=members)
                ...
                if e_parts_sel:
                    if e_split == "Partes iguales":
                        share = e_amount / len(e_parts_sel) if e_parts_sel else 0
                        participants = {m: round(share, 2) for m in e_parts_sel}
                        for m, v in participants.items():
                            st.caption(f"  {m}: ${v:,.2f}")

                    elif e_split == "Por porcentaje":
                        pct_cols = st.columns(len(e_parts_sel))
                        pcts = {}
                        for col, m in zip(pct_cols, e_parts_sel):
                            with col:
                                pcts[m] = st.number_input(f"{m} %", 0.0, 100.0,
                                                            value=round(100/len(e_parts_sel),1),
                                                            step=0.1, key=f"pct_{m}")
                        total_pct = sum(pcts.values())
                        if abs(total_pct - 100) > 0.1:
                            st.error(f"Los porcentajes suman {total_pct:.1f}% — deben sumar 100%")
                            split_ok = False
                        else:
                            participants = {m: round(e_amount * p / 100, 2) for m, p in pcts.items()}

                    else:  # Montos específicos
                        amt_cols = st.columns(len(e_parts_sel))
                        amts = {}
                        for col, m in zip(amt_cols, e_parts_sel):
                            with col:
                                amts[m] = st.number_input(f"{m}", 0.0,
                                                            value=round(e_amount/len(e_parts_sel),2),
                                                            step=1000.0, format="%.2f",
                                                            key=f"amt_{m}")
                        total_amts = sum(amts.values())
                        diff = abs(total_amts - e_amount)
                        if diff > 1:
                            st.error(f"Los montos suman ${total_amts:,.2f} pero el total es ${e_amount:,.2f} (diferencia: ${diff:,.2f})")
                            split_ok = False
                        else:
                            participants = {m: round(v, 2) for m, v in amts.items()}
                ...
                if submitted:
                    ...
                    elif not e_parts_sel:
                        st.error("Selecciona al menos un participante.")
                    elif not split_ok:
                        st.error("Corrige la división antes de guardar.")
                    else:
        ...
        if not expenses:
            ...
            for exp in sorted(expenses, key=lambda x: x.get("date",""), reverse=True):
                parts_str = " · ".join(
                    f"{m}: ${v:,.0f}" for m, v in exp.get("participants", {}).items()
                )
                ec1, ec2 = st.columns([6, 1])
                with ec1:
                    st.markdown(
                        f"**{exp.get('category','')} {exp.get('description','')}** — "
                        f"${exp.get('amount',0):,.0f} · pagó **{exp.get('paid_by','')}** · {exp.get('date','')}\n\n"
                        f"<span style='font-size:.83rem;opacity:.7;'>{exp.get('split_type','')} → {parts_str}</span>",
                        unsafe_allow_html=True,
                    )
                with ec2:
    ...
    with tab_settle:
        ...
        with sc1:
            s_from = st.selectbox("Quien paga", members, index=members.index(username) if username in members else 0)
        with sc2:
            s_to   = st.selectbox("A quién", [m for m in members if m != s_from])
        with sc3:
            s_amt  = st.number_input("Monto", min_value=0.0, step=1000.0, format="%.2f")

        sc4, sc5 = st.columns(2)
        with sc4:
            s_date = st.date_input("Fecha", value=date.today(), key="settle_date")
        with sc5:
            s_type = st.selectbox("Tipo", ["Pago de deuda", "Adelanto"])
        s_note = st.text_input("Nota (opcional)", placeholder="Transferencia, efectivo…")

        if st.button("💾 Registrar pago", type="primary", use_container_width=True, key="btn_settle"):
            if s_amt <= 0:
                st.error("El monto debe ser mayor que 0.")
            else:
                ...
                if storage.save_group_settlements(group_id, sdata, s_sha):
                    st.success(f"✅ Pago registrado: {s_from} → {s_to} · ${s_amt:,.0f}")
                    st.rerun()
                else:
                    st.error("Error al guardar.")

        # Historial de pagos
        settlements_data, _ = storage.get_group_settlements(group_id)
        if settlements_data:
            st.markdown("---")
            st.markdown("**Historial de pagos:**")
            for s in sorted(settlements_data, key=lambda x: x.get("date",""), reverse=True):
                note = f" · {s['note']}" if s.get("note") else ""
                st.markdown(
                    f"<div style='padding:.5rem .9rem;border-radius:9px;"
                    f"background:var(--card-bg);margin:.25rem 0;font-size:.88rem;'>"
                    f"💸 <strong>{s['from_user']}</strong> → <strong>{s['to_user']}</strong> "
                    f"· ${s.get('amount',0):,.0f} · {s.get('date','')} · {s.get('type','')}{note}"
                    f"</div>",
                    unsafe_allow_html=True,
                )
    ...
    with tab_config:
        ...
        # Link de invitación
        base_url    = _get_app_url()
        invite_link = f"{base_url}/?group={group_id}&join=1" if base_url else f"?group={group_id}&join=1"
        st.markdown("**🔗 Enlace de invitación:**")
        st.code(invite_link, language=None)
        ...
        # Solo owner puede editar nombre/imagen o eliminar
        if group.get("owner") == username:
            st.markdown("---")
            st.markdown("**✏️ Editar grupo** _(solo el creador)_")
            ...
            new_img_file = st.file_uploader("Nueva imagen (opcional)", type=["png","jpg","jpeg","webp"], key="cfg_img")
...
def main():
    ...
    # ── Intentar auto-login con token de URL ────────────────────────────────
    if not st.session_state.logged_in:
        if _session_login(st.session_state.storage):
            st.rerun()
            return
