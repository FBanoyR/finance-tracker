...
def _get_budgets(user):
    ...
    Estructura guardada en users.json bajo cada usuario:
      "budgets": {
        "Gasto":    { "base": 2000000, "overrides": { "2025-06": 1500000 } },
        "Ingreso":  { "base": 5000000, "overrides": {} },
        "Inversión":{ "base": 500000,  "overrides": {} }
      }
    'base'      → mensual por defecto (todos los meses sin override)
    'overrides' → meses específicos con monto diferente  (clave "YYYY-MM")
    Compatible con usuarios existentes (sin 'budgets' retorna base=0).
    """
    stored = user.get("budgets", {})
    result = {}
    for tipo in DEFAULT_CATEGORIES:
        b = stored.get(tipo, {})
        result[tipo] = {"base": b.get("base", 0), "overrides": b.get("overrides", {})}
    return result

def _budget_for_month(budgets, tipo, year, month):
    """Presupuesto efectivo de un tipo para un mes concreto.
    Prioridad: override exacto (YYYY-MM) → base mensual → 0."""
    key = f"{year}-{month:02d}"
    b   = budgets.get(tipo, {})
    return b.get("overrides", {}).get(key, b.get("base", 0))
...
def page_dashboard():
    ...
    # ── Gráfico de cumplimiento de presupuestos ────────────────────────────────
    budgets = _get_budgets(st.session_state.user_data)
    has_any_budget = any(b["base"] > 0 or b["overrides"] for b in budgets.values())

    if has_any_budget:
        st.markdown("---")
        st.markdown("#### 🎯 Cumplimiento de Presupuestos por Mes")

        # Construir tabla mes × tipo con real vs presupuesto
        months_in_view = sorted(filtered["mes"].unique())
        rows = []
        for mes_str in months_in_view:
            y, m  = int(mes_str[:4]), int(mes_str[5:7])
            mes_label = f"{MONTHS_ES[m][:3]} {y}"
            mdf   = filtered[filtered["mes"] == mes_str]
            for tipo in DEFAULT_CATEGORIES:
                real  = mdf[mdf["type"] == tipo]["amount"].sum()
                pres  = _budget_for_month(budgets, tipo, y, m)
                met   = _budget_met(tipo, real, pres)
                rows.append({"Mes": mes_label, "mes_ord": mes_str,
                             "Tipo": tipo, "Real": real,
                             "Presupuesto": pres, "met": met})

        bdf = pd.DataFrame(rows)
        bdf_with_budget = bdf[bdf["Presupuesto"] > 0]

        if not bdf_with_budget.empty:
            # ── Barras: real vs presupuesto por tipo ──────────────────────────
            for tipo in DEFAULT_CATEGORIES:
                tdf = bdf_with_budget[bdf_with_budget["Tipo"] == tipo].sort_values("mes_ord")
                if tdf.empty:
                    continue
                ...
                fig_b.add_trace(go.Bar(
                    name="Presupuesto", x=tdf["Mes"], y=tdf["Presupuesto"],
                    marker_color="rgba(180,180,180,0.4)",
                    marker_line=dict(color="rgba(180,180,180,0.8)", width=1.5),
                ))
                fig_b.add_trace(go.Bar(
                    name="Real", x=tdf["Mes"], y=tdf["Real"],
                    marker_color=[
                        "#51CF66" if _budget_met(tipo, r, p) else "#FF6B6B"
                        for r, p in zip(tdf["Real"], tdf["Presupuesto"])
                    ],
                ))
                fig_b.update_layout(
            ...
            total     = len(bdf_with_budget)
            cumplidos = bdf_with_budget.apply(
                lambda r: _budget_met(r["Tipo"], r["Real"], r["Presupuesto"]) is True, axis=1
            ).sum()
            pct = cumplidos / total * 100 if total else 0
            bar_color = "#51CF66" if pct >= 80 else "#FFD43B" if pct >= 50 else "#FF6B6B"
            st.markdown(
                f"""<div style="background:white;border-radius:12px;padding:1rem 1.4rem;
                               box-shadow:0 2px 8px rgba(0,0,0,.07);">
                    <div style="display:flex;justify-content:space-between;margin-bottom:.5rem;">
                        <span><strong>{cumplidos}</strong> de <strong>{total}</strong> meses-tipo cumplidos</span>
...
def page_budgets():
    ...
    for tab, tipo in zip(tabs, list(DEFAULT_CATEGORIES.keys())):
        with tab:
            ...
            # ── Base mensual ──────────────────────────────────────────────────
            st.markdown(f"#### Base mensual — {meta_label}:")
            bc1, bc2 = st.columns([3, 1])
            with bc1:
                new_base = st.number_input(
                    f"Presupuesto base mensual ({tipo})",
            ...
            with bc2:
                if st.button("Guardar base", key=f"save_base_{tipo}",
                             type="primary", use_container_width=True):
                    budgets[tipo]["base"] = float(new_base)
                    changed = True

            if b["base"] > 0:
                st.caption(f"📆 Equivale a **${b['base']*12:,.0f}** anuales")
        ...
        for tipo, color in TYPE_COLORS.items():
            if proj_df[tipo].sum() > 0:
                fig_proj.add_trace(go.Scatter(
                    ...
                    marker=dict(size=8,
                                symbol=["star" if _budget_for_month(budgets, tipo, now.year, m)
                                        != budgets[tipo]["base"] else "circle"
                                        for m in range(1, 13)]),
                ))
