...
def _send_reset_email(to_email: str, nombre: str, code: str) -> bool:
    """
    Envía el código de recuperación usando Resend API.
    Resend es gratuito (3.000 emails/mes) y no requiere cuenta de correo.
    Documentación: https://resend.com/docs
    """
    try:
        api_key      = st.secrets["email"]["resend_api_key"]
        from_address = st.secrets["email"].get("from_address", "FinanceTracker <onboarding@resend.dev>")
    except Exception:
        return False
