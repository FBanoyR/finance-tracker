...
def _render_sidebar():
    ...
    with st.sidebar:
        # Tarjeta de perfil — click lleva a Mi Perfil
        if st.button(
            "",
            key="nav_profile_card",
            ...
            st.rerun()
        st.markdown(
            f"""<div class="sidebar-user" style="margin-top:-2.8rem;pointer-events:none;">
    ...
    with tab_exp:
        ...
        if st.session_state.get("group_adding_expense"):
            ...
            if e_split == "Partes iguales":
                ...
                amts = {}
                default_amt = round(e_amount / len(members), 2) if members and e_amount else 0
                for m in members:
                    ac1, ac2 = st.columns([3, 2])
                    ac1.markdown(f"**{name_of[m]}**")
                    amts[m] = ac2.number_input(
                        "  $", 0.0, value=default_amt,
            ...
            for exp in sorted(expenses, key=lambda x: x.get("date",""), reverse=True):
                ...
                with ec1:
                    st.markdown(
                        ...
                        f"${exp.get('amount',0):,.0f} · pagó **{payer}** · {exp.get('date','')}"
