...
# ── Cookie helpers ────────────────────────────────────────────────────────────
COOKIE_NAME    = "ft_remember_token"
COOKIE_DAYS    = 30

def _get_cookie_manager():
    """Devuelve el CookieManager guardado en session_state (nunca dentro de caché)."""
    if not _COOKIES_AVAILABLE:
        return None
    if "cookie_manager" not in st.session_state:
        st.session_state.cookie_manager = stx.CookieManager(key="ft_cookie_mgr")
    return st.session_state.cookie_manager

def _cookie_login(storage) -> bool:
    """
    Intenta auto-login con la cookie guardada.
    ...
    # Buscar el token en usuarios
    users, _ = storage.get_users()
    for uname, udata in users.items():
        stored = udata.get("remember_token", "")
        exp    = udata.get("remember_expires", "")
        if stored and stored == _hash(token):
...
def _set_remember_cookie(storage, sha, users, username: str):
    """Genera un token seguro, lo guarda en la cookie y en users.json."""
    if not _COOKIES_AVAILABLE:
        return
    cm = _get_cookie_manager()
    if cm is None:
    ...
    users[username]["remember_token"]   = _hash(token)
    users[username]["remember_expires"] = expires.isoformat()
    storage.save_users(users, sha)
    try:
        cm.set(COOKIE_NAME, token, expires=expires)
    except Exception:
        pass

def _clear_remember_cookie(storage, username: str):
    """Elimina la cookie y borra el token del usuario."""
    if not _COOKIES_AVAILABLE:
        return
    cm = _get_cookie_manager()
    if cm is None:
    ...
    if username in users:
        users[username].pop("remember_token",   None)
        users[username].pop("remember_expires", None)
        storage.save_users(users, sha)
...
def _render_sidebar():
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Cerrar sesión", use_container_width=True, key="btn_logout"):
            _clear_remember_cookie(st.session_state.storage,
        ...
        if st.button("Entrar →", use_container_width=True, type="primary", key="btn_login"):
            if not username or not password:
                ...
                if username in users and users[username]["password"] == _hash(password):
                    ...
                    st.session_state.page      = "home"
                    if remember_me:
                        _set_remember_cookie(storage, sha, users, username)
                    st.rerun()
                else:
...
def main():
    ...
    if not st.session_state.logged_in:
        if _cookie_login(st.session_state.storage):
            st.rerun()
            return
