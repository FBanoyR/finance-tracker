...
# ── Cookie + Sessions helpers ─────────────────────────────────────────────────
COOKIE_NAME = "ft_session"
COOKIE_DAYS = 30

def _get_cookie_controller():
    """CookieController singleton en session_state."""
...
def _cookie_login(storage) -> bool:
    """Lee la cookie, valida el token en sessions.json y loguea si es válido."""
    if not _COOKIES_AVAILABLE:
        return False
    ...
    sessions, _ = storage.get_sessions()
    ...
    if datetime.utcnow().isoformat() > exp:
        ...
        storage.save_sessions(sessions, _)
        return False
...
def _set_remember_cookie(storage, username: str):
    """Genera token, lo guarda en sessions.json y lo pone en la cookie."""
    if not _COOKIES_AVAILABLE:
        return
    ctrl = _get_cookie_controller()
    if ctrl is None:
    ...
    sessions = {
        ...
        if v.get("username") != username or v.get("expires", "") > datetime.utcnow().isoformat()
    }
    sessions[token] = {
        "username": username,
    ...
    try:
        ctrl.set(COOKIE_NAME, token, max_age=COOKIE_DAYS * 86400)
    except Exception:
        pass

def _clear_remember_cookie(storage, username: str):
    """Borra la cookie del navegador y elimina el token de sessions.json."""
    if not _COOKIES_AVAILABLE:
        return
    ctrl = _get_cookie_controller()
    if ctrl is None:
        ...
        if st.button("Entrar →", use_container_width=True, type="primary", key="btn_login"):
            if not username or not password:
                ...
                if username in users and users[username]["password"] == _hash(password):
                    ...
                    st.session_state.page      = "home"
                    if remember_me:
                        _set_remember_cookie(storage, username)
                    st.rerun()
                else:
