...
def page_list():
    ...
    for _, row in filtered.iterrows():
        ...
        with st.container():
            ...
            # ── Vista normal ──────────────────────────────────────────────────
                ...
                with col_info:
                    desc_part = (f"<br><span style='font-size:.82rem;opacity:.6;"
                                  f"padding-left:1rem;'>{desc}</span>") if desc else ""
                    st.markdown(
                        f"<div style='padding:.45rem 0;'>"
                        f"<span style='border-left:4px solid {color};padding-left:.6rem;'>"
                        f"<strong>{category}</strong>"
                        f"<span style='opacity:.5;font-size:.8rem;margin-left:.5rem;'>"
                        f"{fecha} · {row['type']}</span></span>"
                        f"{desc_part}</div>",
                        unsafe_allow_html=True,
                    )
                with col_monto:
    ...
    # Espaciador explícito para que el último item no quede bajo el nav bar
    st.markdown(
    ...
    st.download_button("⬇️ Descargar CSV",
                       ...
                       file_name=f"transacciones_{datetime.now():%Y%m%d}.csv",
                       mime="text/csv")
    # Segundo espaciador después del botón
