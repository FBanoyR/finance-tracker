...
def _render_bottom_nav():
    """
    Barra de navegación 100% HTML — nunca se apila en móvil.
    Usa query_params ?nav=key para comunicarse con Streamlit.
    """
    ...
    for icon, label, key in _NAV_ITEMS:
        ...
        items_html += (
            ...
            f"flex:1;padding:6px 2px;border-radius:10px;"
    ...
    st.markdown(
        f"""<div style='display:flex;flex-direction:row;justify-content:space-around;
...
def main():
    ...
    # ── Leer navegación desde query param ?nav=key ────────────────────────────
    nav_param = st.query_params.get("nav", "")
    valid_pages = {"home","add","list","dashboard","categories",
                   "budgets","profile","more","groups","group_detail"}
    if nav_param in valid_pages:
        st.session_state.page = nav_param
        # Limpiar param de nav sin tocar otros (ej. session token)
