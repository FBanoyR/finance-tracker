...
def _send_reset_email(to_email: str, nombre: str, code: str) -> bool:
    """Envía el código de recuperación por correo usando SMTP (Gmail)."""
    import smtplib
    ...
    try:
        ...
        return False  # secrets no configurados
    ...
    msg["Subject"] = f"[FinanceTracker] Código de recuperación: {code}"
    msg["From"]    = smtp_user
    msg["To"]      = to_email
    msg.attach(MIMEText(body_html, "html"))

    # Detecta el proveedor por el dominio del smtp_user
    provider = smtp_user.split("@")[-1].lower() if smtp_user else ""
    try:
        if "gmail" in provider:
            ...
            # Outlook / Hotmail / Office365 (y la mayoría de proveedores SMTP estándar)
            with smtplib.SMTP("smtp.office365.com", 587, timeout=10) as srv:
                srv.ehlo()
                srv.starttls()
                srv.login(smtp_user, smtp_pass)
                srv.sendmail(smtp_user, to_email, msg.as_string())
...
def page_forgot_password():
    ...
    with col:
            ...
            if st.button("Enviar código →", use_container_width=True,
                ...
                if not r_email or "@" not in r_email:
                    ...
                    if not found_user:
                        ...
                        st.success("Si ese correo está registrado, recibirás el código en breve.")
        ...
        # ── Paso 2: verificar código y nueva contraseña ───────────────────────
            ...
            st.info("Revisa tu correo e ingresa el código de 6 dígitos que recibiste.", icon="📬")
