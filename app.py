...
def _inject_css():
    ...
    .nav-item {
        ...
        font-size: .68rem; color: var(--text-color); opacity: .55;
        transition: opacity .15s, background .15s;
        min-width: 52px;
    }
    .nav-item.active { opacity: 1; background: rgba(102,126,234,.12); color:#667eea; }
    ...
    .greeting-bar .g-date {
        font-size: .8rem; color: var(--text-color); opacity: .5;
    }
    </style>
    """, unsafe_allow_html=True)
...
def _render_greeting():
    """Barra minimalista con saludo y botón de logout."""
    ...
    now = datetime.now()
    st.markdown(
        f"""<div class="greeting-bar">
            <span class="g-name">👋 {saludo}</span>
            <span class="g-date">{now.strftime("%d/%m/%Y")}</span>
        </div>""",
        unsafe_allow_html=True,
    )

def _render_bottom_nav():
    """Inyecta la barra de navegación inferior con JS para manejar clicks."""
    page = st.session_state.page
    # Mapeo de página → ícono activo
    ...
    for icon, label, key in _NAV_ITEMS:
        ...
        items_html += (
            f'<button class="{cls}" onclick="window.parent.postMessage('
            f'{{type:\'streamlit:setComponentValue\',value:\'{key}\'}},\'*\')">'
    ...
    # Botones Streamlit invisibles que se activan via session_state
    # (el JS no puede hacer rerun, así que usamos columnas ocultas)
    cols = st.columns(len(_NAV_ITEMS))
    for col, (_, _, key) in zip(cols, _NAV_ITEMS):
        with col:
            if st.button(f"_{key}_", key=f"bnav_{key}",
...
def page_home():
    ...
    # ── Gráfico rápido: evolución 6 meses ─────────────────────────────────────
    df_user["mes"] = df_user["date"].dt.to_period("M").astype(str)
    last6 = sorted(df_user["mes"].unique())[-6:]
    df6   = df_user[df_user["mes"].isin(last6)]
    monthly = df6.groupby(["mes","type"])["amount"].sum().reset_index().sort_values("mes")
    if not monthly.empty:
        fig = px.bar(monthly, x="mes", y="amount", color="type", barmode="group",
                      color_discrete_map=TYPE_COLORS,
                      labels={"amount":"","mes":"","type":""})
        fig.update_layout(
            height=180, margin=dict(l=0,r=0,t=0,b=0),
            plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
            legend=dict(orientation="h", y=1.2, font=dict(size=10)),
            showlegend=True, xaxis=dict(tickangle=-30),
        )
