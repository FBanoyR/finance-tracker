...
def page_dashboard():
    ...
    with r1c2:
        ...
        gdf_pie = filtered[filtered["type"] == "Gasto"]
        if not gdf_pie.empty:
            cat_pie = (gdf_pie.groupby("category")["amount"].sum()
                       .reset_index().sort_values("amount", ascending=False))
            total_gasto = cat_pie["amount"].sum()
            # Mostrar emoji en etiqueta solo si la categoría representa ≥5% del total
            def _pie_label(row):
                pct = row["amount"] / total_gasto * 100 if total_gasto else 0
                # La categoría ya contiene el emoji como primer carácter(es)
                # Para categorías grandes (≥5%) mostramos el nombre completo en la porción
                return row["category"] if pct >= 5 else ""
            cat_pie["label"] = cat_pie.apply(_pie_label, axis=1)
