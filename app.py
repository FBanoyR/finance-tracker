...
def _inject_css():
    ...
    @media (max-width: 768px) {
        ...
        /* Solo apilamos bloques grandes, NO la barra de nav (que tiene 5 columnas) */
        [data-testid="stHorizontalBlock"]:not(:has(> div:nth-child(5))) {
        ...
        .stButton > button { min-height:44px!important; font-size:.95rem!important; }
...
def _render_bottom_nav():
    """
    Nav HTML visual (siempre horizontal en móvil) +
    botones Streamlit ocultos que JS activa al hacer click.
    """
    ...
    for icon, label, key in _NAV_ITEMS:
        bg    = "rgba(102,126,234,0.18)" if active == key else "transparent"
        color = "#667eea"               if active == key else "inherit"
        onclick = f"navTo('{key}')"
        items_html += (
            f"<button onclick='{onclick}' style='"
            f"display:flex;flex-direction:column;align-items:center;"
            f"flex:1;padding:6px 2px;border-radius:10px;border:none;cursor:pointer;"
            f"background:{bg};color:{color};"
            f"font-size:.65rem;font-weight:600;gap:2px;'>"
    ...
    st.markdown(
        f"""
        <div id="ft-nav" style='display:flex;flex-direction:row;
                justify-content:space-around;align-items:center;
                background:var(--card-bg);border-bottom:1px solid var(--card-border);
                padding:6px 4px;margin-bottom:1rem;
                box-shadow:0 2px 8px rgba(0,0,0,.07);border-radius:0 0 12px 12px;'>
            {items_html}
        </div>
        <script>
        function navTo(key) {{
            // Busca el botón Streamlit oculto con data-nav=key y lo clickea
            const hidden = document.querySelector('[data-nav="' + key + '"]');
            if (hidden) {{ hidden.click(); return; }}
            // Fallback: busca por texto del botón
            document.querySelectorAll('button').forEach(b => {{
                if (b.getAttribute('data-nav') === key) b.click();
            }});
        }}
        // Asignar data-nav a cada botón Streamlit oculto por su aria-label
        setTimeout(() => {{
            document.querySelectorAll('button[aria-label^="bnav_"]').forEach(b => {{
                const key = b.getAttribute('aria-label').replace('bnav_', '');
    ...
    # ── Botones Streamlit reales (ocultos visualmente, funcionan al ser clickeados) ──
    st.markdown("""
    <style>
    ...
    for col, (_, label, key) in zip(cols, _NAV_ITEMS):
        with col:
            if st.button(
                label,
                key=f"bnav_{key}",
...
def page_home():
    ...
    has_budget = any(b.get("bases") or b.get("overrides") for b in budgets.values())
    if has_budget:
        st.markdown("**vs Presupuesto este mes**")
        bc1, bc2, bc3 = st.columns(3)
        for col, tipo in zip([bc1, bc2, bc3], DEFAULT_CATEGORIES):
            ...
            color_t = TYPE_COLORS[tipo]
            if pres > 0:
                pct = min(real / pres * 100, 100)
                met = _budget_met(tipo, real, pres)
                bar_c = "#51CF66" if met else "#FF6B6B"
                icon  = "✅" if met else "⚠️"
                resto = pres - real
                if tipo == "Gasto":
                    nota = f"Disponible: ${resto:,.0f}" if resto >= 0 else f"Excedido: ${-resto:,.0f}"
                else:
                    nota = "Meta cumplida" if met else f"Faltan: ${abs(resto):,.0f}"
                html = (
                    f"<div style='background:var(--card-bg);border-radius:12px;"
                    f"padding:.8rem 1rem;border-top:3px solid {color_t};"
                    f"box-shadow:0 1px 6px rgba(0,0,0,.07);'>"
                    f"<div style='display:flex;justify-content:space-between;'>"
                    f"<strong style='font-size:.9rem;'>{tipo}</strong>"
                    f"<span>{icon}</span></div>"
                    f"<div style='font-size:1.1rem;font-weight:700;margin:.3rem 0;'>"
                    f"${real:,.0f} <span style='font-size:.8rem;opacity:.5;'>/ ${pres:,.0f}</span></div>"
                    f"<div style='background:rgba(128,128,128,.15);border-radius:6px;"
                    f"height:6px;overflow:hidden;'>"
                    f"<div style='background:{bar_c};width:{pct:.0f}%;height:100%;border-radius:6px;'></div>"
                    f"</div>"
                    f"<div style='font-size:.75rem;opacity:.6;margin-top:.3rem;'>{nota}</div>"
                    f"</div>"
                )
                ...
                col.markdown(
                    f"<div style='background:var(--card-bg);border-radius:12px;"
                    f"padding:.8rem 1rem;border-top:3px solid {color_t};"
                    f"opacity:.5;text-align:center;font-size:.85rem;'>"
                    f"{tipo}<br>Sin presupuesto</div>",
                    unsafe_allow_html=True,
                )
    ...
    for _, row in df_user.sort_values("date", ascending=False).head(6).iterrows():
        ...
        desc_html = f"<br><small style='opacity:.6;'>{desc}</small>" if desc else ""
        html = (
            f"<div class='tx-card' style='border-left:3px solid {color};padding:.5rem .8rem;'>"
            f"<div style='font-size:.9rem;'><strong>{category}</strong>{desc_html}</div>"
            f"<div style='text-align:right;white-space:nowrap;'>"
            f"<strong style='color:{color};'>{amount}</strong>"
            f"<br><span style='font-size:.75rem;opacity:.5;'>{fecha}</span>"
            f"</div></div>"
        )
...
def page_list():
    """Lista de transacciones con tabla nativa + panel de edición/borrado separado."""
    ...
    transactions, sha = storage.get_transactions()
    if not transactions:
        st.info("Sin transacciones registradas."); return
    ...
    if not filtered.empty:
        # Opciones para el selectbox: "Fecha · Categoría · $monto"
        opciones = {
            row["id"]: f"{pd.Timestamp(row['date']).strftime('%d/%m/%y')} · {row['category']} · ${row['amount']:,.0f}"
            for _, row in filtered.iterrows()
        }
        sel_label = st.selectbox(
            "Selecciona una transacción",
            options=["— ninguna —"] + list(opciones.values()),
            key="sel_tx",
        )
        sel_id = next((k for k,v in opciones.items() if v == sel_label), None)

        if sel_id:
            sel_row = filtered[filtered["id"] == sel_id].iloc[0]
            color   = TYPE_COLORS.get(sel_row["type"], "#888")

            # Info de la transacción seleccionada
            st.markdown(
                f"<div style='background:var(--card-bg);border-left:4px solid {color};"
                f"border-radius:10px;padding:.7rem 1rem;margin:.4rem 0;'>"
                f"<strong>{sel_row['category']}</strong>"
                f" · ${sel_row['amount']:,.0f}"
                f" · {pd.Timestamp(sel_row['date']).strftime('%d/%m/%Y')}"
                f" · {sel_row['type']}"
                f"{'<br><small style=\'opacity:.6;\'>' + str(sel_row.get('description','')) + '</small>' if sel_row.get('description') else ''}"
                f"</div>",
                unsafe_allow_html=True,
            )

            accion = st.radio("Acción", ["✏️ Editar", "🗑️ Eliminar"],
                               horizontal=True, key="tx_accion")

            if accion == "✏️ Editar":
                with st.form("form_edit_tx"):
                    cats_e = user_cats.get(sel_row["type"], [SIN_ASIGNAR])
                    cat_idx = cats_e.index(sel_row["category"]) if sel_row["category"] in cats_e else 0
                    fc1, fc2 = st.columns(2)
                    with fc1:
                        e_type = st.selectbox("Tipo",
                            list(DEFAULT_CATEGORIES.keys()),
                            index=list(DEFAULT_CATEGORIES.keys()).index(sel_row["type"]),
                            key="e_type")
                        e_cat  = st.selectbox("Categoría", cats_e, index=cat_idx, key="e_cat")
                        e_date = st.date_input("Fecha",
                            value=pd.Timestamp(sel_row["date"]).date(), key="e_date")
                    with fc2:
                        e_amount = st.number_input("Monto", min_value=0.0, step=1000.0,
                            format="%.2f", value=float(sel_row["amount"]), key="e_amount")
                        e_desc = st.text_input("Descripción",
                            value=sel_row.get("description","") or "", key="e_desc")
                ...
                st.warning("¿Confirmas que quieres eliminar esta transacción? Esta acción es irreversible.")
                if st.button("🗑️ Sí, eliminar", type="primary", use_container_width=True,
                              key="confirm_del"):
                    all_tx, sha2 = storage.get_transactions()
                    all_tx = [tx for tx in all_tx if tx["id"] != sel_id]
                    if storage.save_transactions(all_tx, sha2):
    ...
    # ── Tabla limpia (nativa, scrollable) ─────────────────────────────────────
...
def page_more():
    ...
    for icon, label, key, desc in options:
        # Botón ancho completo que actúa como la celda completa
        clicked = st.button(
            f"{icon}  {label}",
            key=f"more_{key}",
            use_container_width=True,
        ...
        # Descripción debajo del botón como caption
        st.caption(desc)
        if clicked:
            st.session_state.page = key
            st.rerun()
...
def main():
    ...
    _render_sidebar()   # no-op

    # Barra de navegación HTML al tope
    _render_bottom_nav()
