...
def page_dashboard():
    ...
    # ── Gráfico unificado Real vs Presupuesto ─────────────────────────────────
    ...
    if has_any_budget:
        st.markdown("---")
        st.markdown("#### 🎯 Real vs Presupuesto — historial completo")
        st.caption(
            "Barras = valor real (verde ✅ / rojo ❌ según cumplimiento). "
            "Líneas punteadas = presupuesto. Un mes sin línea = sin presupuesto definido."
        )

        # Generar lista de meses según modo de vista (siempre histórico completo del rango)
        if vista == "Año / Mes":
            # Últimos 12 meses hasta el último mes del año seleccionado
            end_y, end_m = sel_year, 12
            months_range = []
            y, m = end_y - 1, end_m + 1
            for _ in range(12):
                m += 1
            ...
            # Rolling: N meses hacia atrás desde hoy
            months_range = []
            from datetime import date as _date
            ref = datetime.now()
            for i in range(rolling_months - 1, -1, -1):
                month_offset = ref.month - i
                y_off = ref.year + (month_offset - 1) // 12
                m_off = ((month_offset - 1) % 12) + 1
                months_range.append((y_off, m_off))

        # Construir datos por mes × tipo
        rows_b = []
        for (y, m) in months_range:
            ...
            for tipo in DEFAULT_CATEGORIES:
                ...
                rows_b.append({
                    "mes_key": mes_key, "Mes": mes_label,
                    "Tipo": tipo, "Real": real,
                    "Presupuesto": pres, "met": met,
                })

        bdf = pd.DataFrame(rows_b)
        bdf_with = bdf[bdf["Presupuesto"] > 0]

        if not bdf.empty:
            fig_unified = go.Figure()
            mes_labels  = [f"{MONTHS_ES[m][:3]} {y}" for y, m in months_range]

            # Una barra por tipo, coloreada por cumplimiento
            for tipo in DEFAULT_CATEGORIES:
                tdf = bdf[bdf["Tipo"] == tipo].set_index("mes_key").reindex(
                    [f"{y}-{m:02d}" for y, m in months_range]
                ).reset_index()
                tdf["Mes"]  = mes_labels
                tdf["Real"] = tdf["Real"].fillna(0)
                tdf["Presupuesto"] = tdf["Presupuesto"].fillna(0)
                tdf["met"]  = tdf.apply(
                    lambda r: _budget_met(tipo, r["Real"], r["Presupuesto"]), axis=1)

                base_color = TYPE_COLORS[tipo]
                bar_colors = []
                for _, row in tdf.iterrows():
                    if row["met"] is None:
                        bar_colors.append(base_color)          # sin presupuesto → color base
                    elif row["met"]:
                        bar_colors.append("#51CF66")           # cumplido → verde
                    else:
                        bar_colors.append("#FF6B6B")           # incumplido → rojo

                fig_unified.add_trace(go.Bar(
                    name=f"{tipo} real",
                    x=tdf["Mes"], y=tdf["Real"],
                    marker_color=bar_colors,
                    legendgroup=tipo,
                    offsetgroup=tipo,
                ))

                # Línea punteada de presupuesto (solo donde > 0)
                pres_y = [v if v > 0 else None for v in tdf["Presupuesto"]]
                fig_unified.add_trace(go.Scatter(
                    name=f"{tipo} presup.",
                    x=tdf["Mes"], y=pres_y,
                    mode="lines+markers",
                    line=dict(color=base_color, dash="dot", width=2),
            ...
            fig_unified.update_layout(
                ...
                legend=dict(orientation="h", y=1.08, groupclick="toggleitem"),
            ...
            if not bdf_with.empty:
                total     = len(bdf_with)
                cumplidos = bdf_with.apply(
                ...
                st.markdown(
                    f"""<div style="background:white;border-radius:12px;padding:1rem 1.4rem;
                                   box-shadow:0 2px 8px rgba(0,0,0,.07);margin-top:.5rem;">
                        <div style="display:flex;justify-content:space-between;margin-bottom:.5rem;">
                            <span>{emoji} <strong>{cumplidos}</strong> de <strong>{total}</strong>
                            meses-tipo con presupuesto cumplidos</span>
