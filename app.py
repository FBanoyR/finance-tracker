...
def _inject_css():
    ...
    /* ── Slider neutro (gris en lugar del morado primario) ── */
    ...
    div[data-testid="stSlider"] [data-baseweb="slider"] div[class*="fill"] {
        background-color: #888 !important;
    }
...
def _render_bottom_nav():
    """5 botones de navegación en una fila — simples y confiables."""
...
def page_home():
    ...
    # ── Rolling N meses ───────────────────────────────────────────────────────
    st.markdown(
        "<p style='font-size:.85rem;font-weight:600;opacity:.7;margin-bottom:.1rem;'>"
        "📅 Período a visualizar</p>",
        unsafe_allow_html=True,
    )
    n_months = st.slider("Período", 3, 36, 12,
                          key="home_rolling",
...
def main():
    ...
    # Barra de navegación al fondo del contenido
    _render_bottom_nav()
